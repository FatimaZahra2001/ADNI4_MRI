...
from sklearn.metrics import roc_auc_score, balanced_accuracy_score, accuracy_score, f1_score, confusion_matrix, roc_curve
...
class ROIVoxelPatchDataset(Dataset):
    def __init__(
        self,
        df,
        roi_root,
...
class ROIAttentionClassifier(nn.Module):
    def __init__(self, n_rois, in_ch=2, emb_dim=128):
        super().__init__()

        self.encoder = Small3DEncoder(in_ch=in_ch, emb_dim=emb_dim)
...
def plot_attention(attn_rows, roi_names, outpath):
    ...
    means = df[roi_names].mean().sort_values(ascending=True)

    plt.figure(figsize=(7, max(4, 0.4 * len(means))))
    plt.barh(means.index, means.values)
    plt.xlabel("Mean attention weight")
    plt.title("Average ROI attention across validation predictions")
    plt.tight_layout()
    plt.savefig(outpath, dpi=300)
    plt.close()
...
def main():
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(df, y), 1):
        ...
        with torch.no_grad():
            for batch in val_loader:
                x = batch["x"].to(device)
                yb = batch["y"].cpu().numpy().astype(int)
        ...
        fold_auc = roc_auc_score(ys, probs)
    ...
    oof_thr, oof_bacc = best_threshold(all_y, all_prob)
